...
| `apple-design` | **Use**, translated | Ours, hand-written, and **tracked** at `.claude/skills/apple-design/` rather than installed — it is in no lock. Apple's fluid-interface, materials and typography rules, written for the web platform. Three of its defaults lose to decisions here: it says "system font first" (the brand faces in `brand/fonts.json` win, so read its tracking/leading rules *for* Cormorant and Open Sans); its code is CSS, Pointer Events and Motion, which the app cannot run — translate to `Pressable` state, `Animated`/`lineHeight`/`letterSpacing` in a `StyleSheet`; and it wants `backdrop-filter` chrome that content scrolls under, where `AppShell` keeps the header outside the scroller on purpose. |
