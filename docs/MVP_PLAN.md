...
| **D7** | Human email and product email are **separate milestones — and now separate providers** | Human mailboxes were urgent and app-independent; the mailer service depends on the API. Inbound runs on **Google Workspace**, the app's outbound `no-reply@` on **SES** — one apex MX set, so it's exclusive, not additive. See [`EMAIL_SETUP.md`](EMAIL_SETUP.md). |
...
| **1 · Domain & Email** | `insolvia.ai` resolves; `hello@`/`support@`/`security@` are **Google Workspace** mailboxes; the app's `no-reply@` transactional mail sends via **SES** (sandbox-gated until issue 6.8). | Issues `1.x`; runbook [`EMAIL_SETUP.md`](EMAIL_SETUP.md), [`AWS_SETUP.md`](AWS_SETUP.md) |
...
- **Email is split across two providers** — inbound and human replies run on
  **Google Workspace**; the app's outbound `no-reply@` runs on **SES**. Only one
  apex MX set exists, so the two are mutually exclusive, not additive. The old
  SES→Gmail inbound forwarder was built and then removed. Full detail in
  [`EMAIL_SETUP.md`](EMAIL_SETUP.md).
- **SES production access is deferred to last** (issue `6.8`) — deliberately, so
  the request is made with a live site and working bounce handling in place.
  Inbound and human replies are unaffected (they're on Google Workspace); what
  6.8 gates is the **app's** outbound `no-reply@` mail (waitlist, password
  resets), which can only reach verified identities until then — so it shouldn't
  slip past the point the app must email a stranger.
