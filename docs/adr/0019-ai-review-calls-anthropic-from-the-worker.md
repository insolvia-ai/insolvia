...
2. **The Anthropic API is called directly** (the `anthropic` SDK, in the
   worker image only), authenticated by an API key held as the **GitHub
   environment secret `ANTHROPIC_API_KEY`** (environments
   `insolvia-staging` / `insolvia-production`), overlaid onto the Lambda's
   environment by the deploy workflow beside its SSM namespace derivation.
   *(Amended 2026-09-05: originally SSM SecureString
   `/insolvia/<env>/api/anthropic-api-key` — the founder preferred pasting
   the key into GitHub once over running `aws ssm put-parameter`. Trade
   accepted knowingly: the key sits KMS-encrypted in the Lambda environment
   and in GitHub's secret store, and SSM's per-read audit trail is
   forgone.)*
...
- **The local story:** the worker, gate, document builder and findings
  ...
  degrades.
- **The key is the one human-placed secret** (`gh secret set
  ANTHROPIC_API_KEY --env insolvia-staging` / `--env insolvia-production`,
  ...
  not a committed slot: an absent secret means an absent env var means
  the honest failure above, with no placeholder value to mistake for a
  working one. The next api deploy after the secret exists picks it up.
- **The SDK lives in the worker image only** (`requirements-worker.txt`,
  ...
  The API Lambda's environment does carry the variable (one derivation
  plus one overlay, same for both functions) but no API code path reads it.
