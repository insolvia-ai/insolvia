...
test.describe('staging intake', () => {
  test('keeps a half-finished intake across a reload', async ({ page }) => {
    ...
    // it. react-native-web makes every wrapper View a stacking context, and
    // no elevation inside the design system can carry an INLINE list past a
    // wrapper of ours — which is why, since design-system 0.14.1, the open
    // list portals to document.body instead. That mechanism is the package's
    // to keep working; this spec is the app-level proof that it does, on the
    // real intake screen with our real wrappers. Paint order is invisible to
    // the jest suite, which is the reason this lives here.
    //
    // "Requesting a 30-day waiver" is the target because it is measurably the
    // occluded one under the OLD inline rendering: hit-testing that row
    // reached the "Date signed" field wrapper instead (the bottom-most option
    // fell past the Signature section's edge and stayed clickable, so it would
