...
# on the first apply (scripts/bootstrap-ecr-images.sh staging mcp). Its data
...
# already depend on module.mcp_service (they attach its data grants), so the
# service module reading their outputs back would be a cycle. The MCP deploy
# workflow's get-parameters-by-path step derives each into the service's
# environment (last segment, upper-cased, hyphens to underscores).
#
# The SAME tables the api namespace publishes, deliberately re-published
# under /mcp rather than shared: each service's namespace is its complete
# configuration contract, and a deploy that derives one path must never need
# to know another service exists.
...
# The client-id allowlist (#261): derived from the registered harness
# clients, so registering a harness in modules/auth IS adding it to the
# allowlist — never hand-listed. Comma-joined because a Lambda environment
# variable is one string; services/mcp's config splits it back.
