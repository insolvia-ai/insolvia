...
| `scripts/dev-test-unit.sh` | Unit tier (whole repo) | Runs every **unit** suite — pytest per Python unit, Jest/Vitest per JS one, the forms checker — or only the suites a set of files can break (`--list` shows the mapping). **What the pre-push hook runs** (`.pre-commit-config.yaml`); with no arguments it is the "did I break anything anywhere" question before a PR. Unit means unit: nothing it runs needs AWS, a server, a secret or the network. An affected area whose venv is missing FAILS naming its setup script rather than skipping — a hook that skips passes on exactly the machine that was never set up |
| `scripts/dev-fixture.sh` | Seed fixtures (shared bucket) | `publish v1` copies a committed fixture version ([`seeds/fixtures/v1/`](../seeds/fixtures/v1/)) into the account's shared fixture bucket, idempotently by digest; `capture v2 <case-id> <handle>` writes a NEW version out of **this machine's** dev stack (and refuses any other source — that is what keeps every fixture synthetic). Curating a fixture is a human act, so neither runs in CI: the seed role reads the bucket and never writes it |
| `scripts/dev-aws-seed.sh` | Per-machine AWS layer | Makes this machine signable-in **and** seeded, in one run. First ensures the dev account exists in this machine's Cognito pool — there is **no sign-up screen** on any pool (`allow_admin_create_user_only`), so this is the only way to get one; the password comes from `~/.config/insolvia/dev.env`, `DEV_USER_PASSWORD`, or a no-echo prompt that offers to write that file. Then loads [`seeds/dev.json`](../seeds/dev.json) into this machine's dev tables — a firm with your dev account as its **admin**, and the fixture case it names (rows into the case table; the sample documents copied server-side from the shared fixture bucket into this machine's own document bucket). Without the firm a signed-in developer resolves to no firm and every route behind `current_accessor()` answers **403** (`no_active_firm_user`), because the first firm cannot come from the API (`POST /v1/firm/users` is itself behind `FIRM_ADMINISTRATION`). **Edit the fixture to change what is seeded; this script only says where.** The same loader puts `seeds/staging.json` into staging from `.github/actions/seed-staging` (shared by the API's integration job and the app's e2e job), so the two environments differ in fixture and tables, never in the code path that built the rows — which is why the account step lives in this wrapper, not the loader. `--check` verifies both halves |
...
| `scripts/rename-teardown-ecr.sh` | One-time, account-wide | Empties the four pre-rename container repositories so the `shared` apply can destroy them (`aws_ecr_repository` has no `force_delete`, so the apply otherwise dies mid-flight on `RepositoryNotEmptyException` with `shared` half-applied). **Runs BEFORE the shared apply**, and is account-wide rather than per-env — which is why it is not part of `rename-teardown.sh`: emptying these for "staging" would pull the images prod's Lambdas depend on. Opens the outage window; after it, rollback needs a rebuild. `--check`, `--yes`. |
| `scripts/rename-teardown.sh` | One-time, per env | Clears what would make the naming-rename apply fail half-way for ONE environment: DynamoDB deletion protection, Cognito deletion protection, and every object **and version** in the six buckets. Container repositories are the row above, and run earlier. `<staging\|prod> [--check] [--yes]`. **This destroys data — on prod, real case documents and every user's password.** `--check` reports without touching anything; prod additionally requires typing the word `prod`. Run it after `shared` has applied and `bootstrap-ecr-images.sh` has seeded the new repositories, and before that environment's own apply. |
| `scripts/ingest-ust-data.py` | Regulatory registry (means test) | Ingests one UST means-testing dataset release — Census medians (`medians`), IRS National Standards (`national`), IRS Local Standards (`local`) — from justice.gov's per-period XLSX files into `services/api/src/insolvia_api/regulatory/ust/`. Stdlib-only; the written release directory's **diff is the review surface** (effective-dating.md, "The refresh process"). `local` needs `--msa-file` with the hand-curated MSA county lists from the UST's region HTML pages. |
| `scripts/check-ust-data.py` | Regulatory registry (means test) | The scheduled freshness tripwire: compares the UST's current means-testing period (file digests) against the committed registry, and watches the § 104 dollar-amount horizon. Run weekly by `regulatory-refresh.yml`, which opens a GitHub issue on failure so the alert reaches a human; runnable locally any time. Exit 0 current · 1 stale · 2 cannot verify. |
...
| `forms/scripts/dev-test.sh` | Form field specs | Validates `forms/specs/*.json` against the AcroForm dumps in `forms/acroform/` — structure, entity vocabulary, full coverage of every fillable widget in the official PDFs. Stdlib-only Python, no venv and no setup script; exactly what `forms-pr.yml` runs. See [`forms/README.md`](../forms/README.md) |
| `services/api/scripts/dev-test-integration.sh` | API integration tier, against local dev | Runs `services/api/tests/integration` — the RUNNING API over HTTP, signed in by SRP as the person [`seeds/dev.json`](../seeds/dev.json) names — against `http://localhost:8080` and this machine's pool. Needs `scripts/dev-up.sh` running and `scripts/dev-aws-seed.sh` done; the password comes from `E2E_TEST_USER_PASSWORD` or `~/.config/insolvia/dev.env`; no AWS credentials are used. `api-staging.yml` runs the same tier against staging after every deploy. A bare `pytest` in that service is the unit tier only |
| `e2e/scripts/dev-test.sh` | E2E, against local dev | Runs both Playwright projects (`flows`, signed in; `smoke`, no credentials) against `http://localhost:3000` / `:8080` and **this machine's** dev Cognito pool, instead of deployed staging. Needs `scripts/dev-up.sh` running and `scripts/dev-aws-seed.sh` done; the password comes from `E2E_TEST_USER_PASSWORD` if exported, else `~/.config/insolvia/dev.env` — no committed default, the repo is public. The address comes from [`seeds/dev.json`](../seeds/dev.json). `--headed` to watch it, `--project smoke` for the unauthenticated half. The staging run in `app-staging.yml` stays authoritative; `app-prod.yml` runs `smoke` alone against production |
