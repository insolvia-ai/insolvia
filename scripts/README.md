...
| `scripts/dev-aws-create-firm.sh` | Per-machine AWS layer | Creates a firm in this machine's dev tables and makes that account its **admin**. Without it a signed-in developer resolves to no firm and every route behind `current_accessor()` answers **403** (`no_active_firm_user`). The first firm cannot come from the API — `POST /v1/firm/users` is itself behind `FIRM_ADMINISTRATION` — so it is seeded here, through `services/api`'s own `core/firms.py`; `--check` verifies |
| `scripts/dev-aws-reset.sh` | Per-machine AWS layer | Wipes this machine's dev **data** — waitlist, case, access-log and firm tables (delete + recreate) plus the pool's users — resources survive; `--dry-run`, `--skip-cognito`. Leaves you needing `dev-aws-create-user.sh` **and** `dev-aws-create-firm.sh` again (only the latter with `--skip-cognito`) |
...
- **And a firm before that account can do anything.** These are two separate
  ...
  Skip `./scripts/dev-aws-create-firm.sh` and the app renders, `/v1/me` reports
