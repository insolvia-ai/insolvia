...
| `scripts/dev-aws-create-staff-user.sh` | Per-machine AWS layer | Creates a STAFF account in this machine's dev **staff** pool (`insolvia-staff-dev-<id>`, #209) — the admin portal's issuer, separate from the firm pool above. Same password handling and fences as its sibling; the staff pool **requires TOTP**, enrolled at first sign-in. `--check` verifies |
