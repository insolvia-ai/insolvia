...
| `scripts/dev-aws-seed.sh` | Per-machine AWS layer | Writes the dev rows the API cannot create itself — `dev-aws-seed.sh <what>`, today just `firm`. `firm` creates a firm and makes this machine's dev account its **admin**; without it a signed-in developer resolves to no firm and every route behind `current_accessor()` answers **403** (`no_active_firm_user`), because the first firm cannot come from the API (`POST /v1/firm/users` is itself behind `FIRM_ADMINISTRATION`). This script resolves the environment; the rows are built by `services/api`'s own `core/` through `insolvia_api.entrypoints.seed`, so a seeded row cannot drift from what the service reads. `--check` verifies |
| `scripts/dev-aws-reset.sh` | Per-machine AWS layer | Wipes this machine's dev **data** — waitlist, case, access-log and firm tables (delete + recreate) plus the pool's users — resources survive; `--dry-run`, `--skip-cognito`. Leaves you needing `dev-aws-create-user.sh` **and** `dev-aws-seed.sh firm` again (only the latter with `--skip-cognito`) |
...
- **And a firm before that account can do anything.** These are two separate
  ...
  Skip `./scripts/dev-aws-seed.sh firm` and the app renders, `/v1/me` reports
