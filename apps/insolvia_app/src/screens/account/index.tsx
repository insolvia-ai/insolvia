...
/**
 * Your own account: the display name your colleagues see, and the email you
 * sign in with.
 *
 * The name goes to `PATCH /v1/me` — the one field a member may change about
...
export function Account({ membership }: { membership: FirmMembership }) {
  ...
  const submit = async () => {
    ...
    setNotice(null);
    try {
      const result = await call((client) => client.updateMe({ displayName }));
      if (result.ok) {
        ...
        setDisplayName(result.value.firm?.displayName ?? displayName);
