#!/usr/bin/env bash
#
# Stop the marketing dev server — the counterpart to dev-up.sh.
#
# Unlike the compose services there is no container to remove; what this
# reclaims is the PORT. Vite's dev server can outlive the shell that started
# it, and the failure is quieter than a bind error: Vite simply picks the next
# free port, so the site comes up somewhere nobody is looking.
#
