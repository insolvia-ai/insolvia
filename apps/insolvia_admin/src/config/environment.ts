 ...
 * Internal OAuth client with http://localhost:3100 among its Authorized
 * JavaScript origins — see infra/envs/dev/variables.tf, which commits the
 * same value for the service side).
 */
...
const CONFIGS: Record<InsolviaEnvironment, AdminConfig> = {
  local: {
    environment: 'local',
    googleClientId: '925851246989-tttlgepq8mjfhmtlkmgikrtv4kskv0pk.apps.googleusercontent.com',
    apiBaseUrl: 'http://127.0.0.1:8090',
  },
  staging: {
    environment: 'staging',
    googleClientId: '925851246989-a4prtrjp0p5j1q71g8pbv4irqu7ibsce.apps.googleusercontent.com',
    apiBaseUrl: 'https://staging-admin-api.insolvia.ai',
  },
  production: {
    environment: 'production',
    googleClientId: '925851246989-115l1fsln1ntv52uv3bhg29k819fram7.apps.googleusercontent.com',
...
export function resolveConfig(raw: string | undefined): AdminConfig {
  ...
  if (config === undefined) {
    throw new Error(`VITE_INSOLVIA_ENV is "${name}" but must be local, staging, or production`);
  }
  return config;
}
