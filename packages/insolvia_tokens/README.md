...
until the two stacks disagree.

The banners are not identical, deliberately: the four Dart outputs name the
regeneration command, while `theme.css` points back at this README. `theme.css`
is the one output that lands inside a version-gated design-system package, so
its bytes are expensive — keeping the command out of it means re-implementing
the generator rewrites no generated bytes and needs no design-system PR.
