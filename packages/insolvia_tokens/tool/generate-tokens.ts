...
//
// `theme.css` lands inside `insolvia_design_system_react`, a version-gated
// package, so its bytes are expensive: rewriting one would drag an otherwise
// unrelated change into `insolvia-design-system-pr` territory (own PR + version
// bump) for no user-visible gain. Naming the README rather than a command keeps
// the banner toolchain-agnostic, and that is exactly what let this generator be
// re-implemented in another language without rewriting a single generated byte
// and without a design-system PR.
const CSS_BANNER =
