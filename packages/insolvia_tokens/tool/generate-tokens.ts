...
// Both generated files name the regeneration command. Neither lands in a
// version-gated package any more — the marketing site owns its `theme.css`
// directly (no separate design-system package to publish), and `tokens.ts`
// lives here — so there is nothing to keep the banner toolchain-agnostic for.
const CSS_BANNER =
...
// `theme.css` is Tailwind v4's `@theme` layer for the marketing site, imported
// straight from its CSS entrypoint. It used to be published inside a separate
// design-system package; the site now owns its own themed components, so the
// generated theme goes to the site.
const CSS_OUT = 'apps/insolvia_marketing/app/styles/theme.css';
 ...
 *
 * The marketing site imports this from its Tailwind v4 CSS entrypoint:
 *
 *   @import "tailwindcss";
 *   @import "./theme.css";
 *
