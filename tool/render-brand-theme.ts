...
import type { FontRole, Palette, RadiusStep, Scheme } from './brand-palette.ts';
...
async function main(args: string[]): Promise<void> {
  ...
  const outputs: ReadonlyArray<readonly [string, string]> = [
    [APP, await pretty(appModule(brand, families, corners), APP)],
    [MARKETING, await pretty(stylesheet(brand, families, corners, MARKETING), MARKETING)],
    [ADMIN, await pretty(stylesheet(brand, families, corners, ADMIN), ADMIN)],
  ];
  ...
  if (drifted.length === 0) {
    if (check)
      process.stdout.write(
        `brand themes are in sync with ${BRAND}, ${BRAND_FONTS} and ${BRAND_RADII}.\n`,
      );
    return;
  }

  process.stderr.write(
    `These generated themes have drifted from ${BRAND}, ${BRAND_FONTS} or ${BRAND_RADII}:\n\n`,
  );
...
function appModule(
  ...
  corners: Readonly<Record<RadiusStep, number>>,
): string {
  const lines: string[] = [
...
function stylesheet(
  ...
  // Corners are scheme-independent too, so they join the families in @theme.
  for (const step of RADIUS_STEPS) {
    lines.push(`  --radius-${step}: ${corners[step]}px;`);
  }
