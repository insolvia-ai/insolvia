#!/usr/bin/env bash
#
# Run the admin service's lint + format + test gate locally — the same
# commands, in the same order, as the `Admin service` job in
# .github/workflows/admin-pr.yml:
#   ruff check .  →  ruff format --check .  →  mypy  →  pytest
# (CI additionally builds the Lambda image; run that separately with
#  `docker build --target lambda -f services/admin/Dockerfile .` FROM THE
...
if [[ ! -x "$VENV/bin/python" ]]; then
  printf '\033[1;33m[warn]\033[0m venv missing — run ./services/admin/scripts/dev-setup.sh first.\n' >&2
  exit 1
fi
