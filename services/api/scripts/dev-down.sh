#!/usr/bin/env bash
#
# Stop the API's local stack — the counterpart to dev-up.sh.
#
# This exists because killing the process that ran `docker compose up` does
# NOT reliably stop its containers: one is left holding port 8080, and the
...
printf '\033[1;32m[ ok ]\033[0m API stack is down.\n'
